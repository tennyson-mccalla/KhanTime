...
// Populate all 3 Khan Academy units with proper resources
...
// Create a resource with proper format
func createResource(resourceId: String, title: String, url: String, token: String) throws {
...
// Main execution
do {
    print("🚀 Populating all Khan Academy units with real content...")
    ...
    for (unitIndex, unit) in unitsToProcess.enumerated() {
        ...
        for (lessonIndex, lesson) in unit.lessons.enumerated() {
            let resourceId = "khan-lesson-\(unitIndex)-\(lessonIndex)-\(timestamp)"
            ...
            // Associate with unit
            let componentResourceId = "khan-comp-res-\(unitIndex)-\(lessonIndex)-\(timestamp)"
        ...
        // Process exercises (quizzes) in this unit
        for (exerciseIndex, exercise) in unit.exercises.enumerated() {
            let resourceId = "khan-quiz-\(unitIndex)-\(exerciseIndex)-\(timestamp)"
            let url = "https://www.khanacademy.org/math/pre-algebra/\(exercise.slug)"
            ...
            // Associate with unit
            let componentResourceId = "khan-quiz-comp-res-\(unitIndex)-\(exerciseIndex)-\(timestamp)"
            try associateResourceToComponent(
                componentResourceId: componentResourceId,
                unitId: unitId,
                resourceId: resourceId,
                resourceTitle: exercise.title,
                sortOrder: resourceCount,
                token: token
            )
        ...
        print("✅ Completed unit: \(unit.title) (\(resourceCount) resources)")
    }
    
    print("\n🎉 SUCCESS!")
    print("📱 All Khan Academy units now have real content!")
